...
@version: 1.0
@author: zxd3099
@file: mossformer_conv_module
@time: 2023-06-23 22:37
"""
import torch
...
class GlobalLayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        :param x: Tensor of size [N, C, K, S] or [N, C, L].
        :return:
        """
        if x.dim() == 3:
...
class CumulativeLayerNorm(nn.LayerNorm):
    ...
    def forward(self, x):
        ...
        @:param x: Tensor size [N, C, K, S] or [N, C, L]
        """
        # N x K x S x C
        if x.dim() == 4:
...
class MossFormerConvModule(nn.Module):
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        """
        @:param inputs (batch, time, dim): Tensor contains input sequences

        @:return outputs (batch, time, dim): Tensor produces by conformer convolution module.
        """
